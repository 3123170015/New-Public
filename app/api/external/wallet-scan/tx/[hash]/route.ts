...
export async function GET(req: Request, context: { params: Promise<{ hash: string }> }) {
  const key = await requireApiKey(req, ["wallet-scan/read"]);
  if (key instanceof Response) return key;
  ...
  if (!hash) {
    return Response.json({ ok: false, error: "TX_HASH_REQUIRED" }, { status: 400, headers: key.cors });
  }
  ...
  const user = await resolveWalletScanUser({ txHash: hash });
