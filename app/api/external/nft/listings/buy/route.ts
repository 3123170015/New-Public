...
export async function POST(req: Request) {
  ...
  try {
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      await releaseMaturedHoldsTx(tx, result.user.id);
      ...
      if (listing.sellerId === result.user.id) throw new Error("CANNOT_BUY_OWN_LISTING");
      ...
      const buyer = await tx.user.findUnique({ where: { id: result.user.id }, select: { id: true, starBalance: true } });
      ...
      await tx.user.update({ where: { id: result.user.id }, data: { starBalance: { decrement: priceStars } } });
