...
export async function POST(req: Request) {
  ...
  const rl = await rateLimit(
    `topup:submit-tx:user:${userId}`,
    Number(process.env.TOPUP_SUBMIT_TX_LIMIT || 8),
    Number(process.env.TOPUP_SUBMIT_TX_WINDOW_MS || 10 * 60 * 1000)
  );
  if (!rl.ok) {
  ...
  if (existing && existing.userId && existing.userId !== userId) {
    ...
    await prisma.starDepositEvent.create({
      data: { depositId: dep.id, type: "FRAUD_DUP_TX_HASH", message: `Duplicate txHash detected: also in deposit ${existing.id}` },
    });
    await prisma.starDeposit.update({
      where: { id: dep.id },
      data: { status: "NEEDS_REVIEW", failureReason: "dup-tx-hash", txHash: parsed.data.txHash },
    });
